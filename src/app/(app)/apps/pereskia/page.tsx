...
import { PereskiaApp } from '@/components/cactus/apps/pereskia/pereskia-app';
...
export const metadata = { title: 'Pereskia · Música & Audio · Cactus' };

export default async function PereskiaAppPage() {
  const agent = getAgent('pereskia');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/pereskia');
