...
import { SanPedroApp } from '@/components/cactus/apps/sanpedro/sanpedro-app';
...
export const metadata = { title: 'San Pedro · Animación · Cactus' };

export default async function SanPedroAppPage() {
  const agent = getAgent('sanpedro');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/sanpedro');
