...
import { FerocactusApp } from '@/components/cactus/apps/ferocactus/ferocactus-app';
...
export const metadata = { title: 'Ferocactus · Contratos & Documentos · Cactus' };

export default async function FerocactusAppPage() {
  const agent = getAgent('ferocactus');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/ferocactus');
