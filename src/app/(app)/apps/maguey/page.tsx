...
import { MagueyApp } from '@/components/cactus/apps/maguey/maguey-app';
...
export const metadata = { title: 'Maguey · Ventas & Propuestas · Cactus' };

export default async function MagueyAppPage() {
  const agent = getAgent('maguey');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/maguey');
