...
import { TunaApp } from '@/components/cactus/apps/tuna/tuna-app';
...
export const metadata = { title: 'Tuna · CRM & Revenue · Cactus' };

export default async function TunaAppPage() {
  const agent = getAgent('tuna');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/tuna');
