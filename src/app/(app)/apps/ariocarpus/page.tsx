...
import { AriocarpusApp } from '@/components/cactus/apps/ariocarpus/ariocarpus-app';
...
export const metadata = { title: 'Ariocarpus · Avatares & Humanos Digitales · Cactus' };

export default async function AriocarpusAppPage() {
  const agent = getAgent('ariocarpus');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/ariocarpus');
