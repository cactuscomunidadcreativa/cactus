...
import { AloeApp } from '@/components/cactus/apps/aloe/aloe-app';
...
export const metadata = { title: 'Aloe · Atención al Cliente · Cactus' };

export default async function AloeAppPage() {
  const agent = getAgent('aloe');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/aloe');
