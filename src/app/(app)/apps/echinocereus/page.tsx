...
import { EchinocereusApp } from '@/components/cactus/apps/echinocereus/echinocereus-app';
...
export const metadata = { title: 'Echinocereus · SEO & Growth · Cactus' };

export default async function EchinocereusAppPage() {
  const agent = getAgent('echinocereus');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/echinocereus');
