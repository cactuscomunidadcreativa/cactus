...
import { OcotilloApp } from '@/components/cactus/apps/ocotillo/ocotillo-app';
...
export const metadata = { title: 'Ocotillo · Talento & Reclutamiento · Cactus' };

export default async function OcotilloAppPage() {
  const agent = getAgent('ocotillo');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/ocotillo');
