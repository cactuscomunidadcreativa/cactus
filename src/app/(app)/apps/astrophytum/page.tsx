...
import { AstrophytumApp } from '@/components/cactus/apps/astrophytum/astrophytum-app';
...
export const metadata = { title: 'Astrophytum · Personajes · Cactus' };

export default async function AstrophytumAppPage() {
  const agent = getAgent('astrophytum');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/astrophytum');
