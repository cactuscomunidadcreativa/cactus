...
import { CandelabroApp } from '@/components/cactus/apps/candelabro/candelabro-app';
...
export const metadata = { title: 'Candelabro · Video · Cactus' };

export default async function CandelabroAppPage() {
  const agent = getAgent('candelabro');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/candelabro');
