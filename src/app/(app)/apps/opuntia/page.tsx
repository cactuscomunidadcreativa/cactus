...
import { OpuntiaApp } from '@/components/cactus/apps/opuntia/opuntia-app';
...
export const metadata = { title: 'Opuntia · Web Builder · Cactus' };

export default async function OpuntiaAppPage() {
  const agent = getAgent('opuntia');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/opuntia');
