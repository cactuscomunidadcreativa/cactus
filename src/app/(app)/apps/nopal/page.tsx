...
import { NopalApp } from '@/components/cactus/apps/nopal/nopal-app';
...
export const metadata = { title: 'Nopal · Social Media · Cactus' };

export default async function NopalAppPage() {
  const agent = getAgent('nopal');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/nopal');
