...
import { ChollaApp } from '@/components/cactus/apps/cholla/cholla-app';
...
export const metadata = { title: 'Cholla · Campañas & Performance · Cactus' };

export default async function ChollaAppPage() {
  const agent = getAgent('cholla');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/cholla');
