...
import { YucaApp } from '@/components/cactus/apps/yuca/yuca-app';
...
export const metadata = { title: 'Yuca · Bienestar & Hábitos · Cactus' };

export default async function YucaAppPage() {
  const agent = getAgent('yuca');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/yuca');
