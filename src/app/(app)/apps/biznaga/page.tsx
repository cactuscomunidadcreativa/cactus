...
import { BiznagaApp } from '@/components/cactus/apps/biznaga/biznaga-app';
...
export const metadata = { title: 'Biznaga · Market Intelligence · Cactus' };

export default async function BiznagaAppPage() {
  const agent = getAgent('biznaga');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/biznaga');
