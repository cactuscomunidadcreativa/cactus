...
import { LenteApp } from '@/components/cactus/apps/lente/lente-app';
...
export const metadata = { title: 'Lente · Fotografía IA · Cactus' };

export default async function LenteAppPage() {
  const agent = getAgent('lente');
  if (!agent) notFound();
  ...
  if (supabase) {
    ...
    if (!u) redirect('/login?redirect=/apps/lente');
